...
export const handler: Handler = async (event: HandlerEvent, _) => {
  ...
  if (
    ...
    isInvalid(form.groupName) ||
    isInvalid(form.dateRequested) ||
    isInvalid(form.hirePeriod) ||
    isInvalid(form.otherInformation)
  ) {
    return errorResponse(process.env.DOMAIN);
  }
...
async function sendEmail(
  ...
  const message: EmailMessage = {
    ...
    content: {
      subject: `New Website Enquiry (${form.dateRequested} - ${form.hirePeriod})`,
      html: emailBody(form),
    },
    recipients: {
      to: [...recipients],
